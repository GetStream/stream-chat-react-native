...
- `MediaList` and `ImageGrid` key a `FlatList` remount on column count. RN does not support changing `numColumns` on a mounted list, so this is required - but crossing the 600pt breakpoint (i.e. a fold) costs scroll position.
...
> - The media grid in channel details now picks its column count from its own width instead of a hardcoded 3, and accepts a `numberOfColumns` prop. Phone-width layouts are unchanged.
...
change to `numColumns` on a mounted list. Crossing the breakpoint therefore resets scroll position -
precisely the kind of jump this guidance warns against. The even-column change reduces how often the
count changes but does not remove the remount. Options, none free: capture and restore the scroll
offset across the remount, or replace `FlatList` with a `flexWrap` layout that can reflow in place.
Recorded rather than actioned, because it is a performance-sensitive list and the right answer is a
maintainer's call.
