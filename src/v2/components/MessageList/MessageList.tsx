...
>(
  ...
  const isUnreadMessage = useCallback(
    (
  ...
  const updateStickyHeaderDateIfNeeded = useCallback(
  ...
  useEffect(() => {
    ...
    if (initialScrollToFirstUnreadMessage && !initialScrollSet.current) {
      ...
      topMessage.current = messageList[messageList.length - 1];
    }
  }, [messageList.length]);
  ...
  const stickyHeaderFormatDate = useMemo(
    () =>
  ...
  const tStickyHeaderDate = useMemo(() => tDateTimeParser(stickyHeaderDate), [
    stickyHeaderDate,
  ]);
  const stickyHeaderDateToRender = useMemo(
    () =>
  ...
  const dismissImagePicker = useCallback(() => {
    if (!hasMoved && selectedPicker) {
      setSelectedPicker(undefined);
      closePicker();
    }
  }, [hasMoved, selectedPicker]);
