...
Stream Chat for React Native uses contexts to distribute data, functions, and components through the SDK.
You can use these contexts when creating custom components to use instead of out of the box components.
...
We suggest using the hooks that are provided by the library to access information from these contexts.
