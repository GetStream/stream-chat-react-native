...
[`takePhoto`](#takephoto) handler is used to take a picture from camera and use it as image attachment which composing a message.
...
Although its possible to replace it with custom implementation, using [react-native-image-picker](https://github.com/react-native-image-picker/react-native-image-picker) library in following example:
