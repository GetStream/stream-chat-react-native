...
:::note
Do not install the `stream-chat` dependency externally. This may lead to problems while using our SDK. Our SDK has it as a dev dependency, so you need not install it externally.
:::
