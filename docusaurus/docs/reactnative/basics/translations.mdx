...
`Streami18n` provides static translation inside other Stream Chat components.
`Streami18n` is a class that exposes a subset of [i18next](https://www.i18next.com/) configuration.
You can import `Streami18n` from the `stream-chat-react-native` library:
...
To create an instance of `Streami18n` you simply need to instantiate the class. `Streai18n` doesn't require any parameters for creation and will default to English (en).

```ts
const streami18n = new Streami18n();
```

`Streami18n` accepts two parameters when you instantiate it, [`options`](#options) and [`i18nextConfig`](#i18nextconfig).
...
<!-- Is usage of this with Day.js so common that we should start the section on usage with it? -->
If you're using default English settings with Day.js to handle dates, you do not need to deal directly with `Streami18n`. 

You will need to provide a modified instance of `Streami18n` to the component library if you choose to change one of the following settings:
 
- language 
- translation
- date handling

Two components require your custom instance of `Streami18n` to properly pass your translation and time-date functions to the component library:  [`OverlayProvider`](docusaurus/docs/reactnative/core-components/overlay_provider.mdx) and [`Chat`](docusaurus/docs/reactnative/core-components/chat.mdx).
Both components accept an instance of `Streami18n` via the prop `i18nInstance`.
Passing this prop  your instance of `Streami18n` will provide that istance to the other components via [`context`](https://reactjs.org/docs/context.html) instead of the default instance.
