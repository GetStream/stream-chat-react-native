...
On Android if the StatusBar is set to translucent, that is `StatusBar.setTranslucent(true)`, you can simply add the height/padding adjustments on the container wrapping the `Channel` component.
This will adjust the differences in the measurements of the screen height when the StatusBar on Android is translucent.
