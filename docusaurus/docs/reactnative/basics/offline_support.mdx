...
Chat React Native SDK replies on default caching provided by react-native (avatars, image attachments, url preview etc) by default.
Although its not really perfect and you might see a lot of images missing while operating app in offline mode.
But its easy to replace all the underlying usage of [`Image`](https://reactnative.dev/docs/next/image) component using one
simple prop on `Chat` component - `ImageComponent`
You can provide a custom component to display image which handles caching and has the same API as react-native's `Image` component.
...
Following example demonstrates the usage of react-native-fast-image library to handle caching of images in chat:
