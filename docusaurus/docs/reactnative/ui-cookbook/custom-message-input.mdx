...
with upload previews ([`ImageUploadPreview`](../ui-components/message-input-components/image-upload-preview.mdx)) within `MessageInput` component
by using a function `uploadNewImage` provided by [`MessageInputContext`](../../ui-components/contexts/message-input-context#uploadnewimage).

For file attachments, the SDK defaults to the native file picker. That can be reused by adding the `pickFile` function
provided by the [`MessageInputContext`](../../ui-components/contexts/message-input-context#pickfile) as an action handler within the action sheet.
