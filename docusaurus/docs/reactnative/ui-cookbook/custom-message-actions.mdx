---
id: custom-message-actions
sidebar_position: 4
title: Custom Message Actions
---

import messageActions from '../assets/ui-cookbook/custom-message-actions/message_actions.png';
import myMessageActions from '../assets/ui-cookbook/custom-message-actions/my_message_actions.png';
...
Every message action that you see in UI, is represented by [`MessageAction`](../state-and-offline-support/object-types.mdx#messageaction) object for that action. And `MessageAction` object provides all the necessary inputs (title, icon, action handler, `actionType`) for rendering the action button.
...
You can customize each one of the default actions using props on the [`Channel component`](../../ui-components/channel) as shown further on this page. The channel component makes these props available in the `MessagesContext` context.
...
[`OverlayProvider`](../ui-components/overlay-provider-components/overlay-provider.mdx) component accepts props called - `MessageActionList` and `MessageActionListItem`. They both serve a different purpose.
