...
Our `AutoCompleteSuggestionList` already support emoji search using a query that starts with `:` in the message input, on a default data set that can be found [here](https://github.com/GetStream/stream-chat-react-native/blob/develop/package/src/emoji-data/compiled.ts).
