...
Since the `notification` field is present on iOS, it is automatically picked up by the Firebase SDK and the notification is displayed to the user when the app is not on foreground.

:::info

If we do want to customise the notification dynamically on the iOS app, say increment the badge number on every push for example, we will need to implement a [`Notification Service Extension`](https://developer.apple.com/documentation/usernotifications/unnotificationserviceextension) in the native language.
...
This handler works only on Android. For this handler to work on iOS, the payload must be made `data` only. See [pushing background updates to your app](https://developer.apple.com/documentation/usernotifications/setting_up_a_remote_notification_server/pushing_background_updates_to_your_app) part in the Apple iOS documentation.

:::

 When the handler is triggered we can display the notification on Android after retreiving the message. To setup a background handler, call the `setBackgroundMessageHandler` outside of your application logic as early as possible:
...
When a user interacts with the notification by pressing on it, the default behavior is to open the application. In many cases, it is useful to detect whether the application was opened by pressing on a notification (so you could open a specific screen for example). The [React Native Firebase API](https://rnfirebase.io/messaging/notifications#handling-interaction) provides two APIs for handling interaction:
...
### Add notification field to payload for Android

To update the payload for Android to have notification payload also, we can add relevant fields to the `notification_template` using the JavaScript SDK like below:
...
### Make iOS payload to be data only

If iOS payload is also made data only, then the `setBackgroundMessageHandler` can be used to display notifications using notifee. This can be done using the JavaScript SDK like below:
