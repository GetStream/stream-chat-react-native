...
import loadMoreButton from '../assets/guides/customize-channel-list/load_more_button.png'

In this document, we will cover various use cases of customizations around [`ChannelList`](../core-components/channel_list.mdx).


## Customize event handlers?

`ChannelList` uses [Event Listeners](https://getstream.io/chat/docs/javascript/ev\ent_listening/?language=javascript) to dynamically update when changes occur.
...
<ChannelList 
  additionalFlatListProps={{
    ListFooterComponent: FotterLoadMoreButton,
    onEndReached: () => null
  }}
/>
```
