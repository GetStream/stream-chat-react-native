...
Guide to help with upgrading from v2 to v3 of `stream-chat-react-native`.

Version 3 of React Native SDK has been a major architectural change. The new implementation takes advantage of React Context along with many popular
...
Before you begin the upgrade, we would recommend you to comment out all the component customizations related props on `ChannelList`, `Channel` or `MessageList` that you may have in your application.
...
to provide custom component for rendering a message UI. But in v3, you need to provide `MessageSimple` prop on Channel component instead.
...
Please check documentation of [contexts](../customizations/contexts.mdx) for all the available contexts.

The customization of MessageFooter will look like following after upgrade to v3.
