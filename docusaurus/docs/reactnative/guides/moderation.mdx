...
:::note
When you try to resend a message when bounced, if the bounced then flag settings is applied, the message will be flaged after being sent the second time.
...
You can customize the default actions on the bounced message action handler component i.e. [MessageBounce](../ui-components/message-bounce.mdx) component by passing it to the [`Channel`](../core-components/channel.mdx#messagebounce) component.
