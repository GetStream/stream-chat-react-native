...
You can customize each one of the default actions using prop provided to OverlayProvider (as shown further on this page). OverlayProvider component makes these props available to enclosed components via context `MessageOverlayContext`.
...
- `MessageActionList` - Allows full customizability of the message action modal and allows users to add/define their own message action along with the style they prefer for the application.
- `MessageActionListItem` - Allows customizability to the already available actions which are present by default. Users can change the UI as well as change the on click action according to their own use case.
...
An example for the `MessageActionListItem` component rendering goes here:
...
## How to conditionally change a message action?

Additionally, the following example demonstrates how you can add custom title, icon or some other component as per your requirement.
...
The types of actions which are available are as follows:
...
By default if you don't have any condition you need to return the `MessageActionListItem` component and pass in the action props to it so that it will be rendered.
