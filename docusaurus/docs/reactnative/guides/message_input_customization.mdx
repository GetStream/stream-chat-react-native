...
Attachment picker opens up as bottom sheet by default.
It's easy to replace this default attachment picker with a device's native image picker, as shown in screenshots below.
In this section, we will walk through the integration of native image picker to attach images in message.
...
Channel component takes a prop [AttachButton](../core-components/channel.mdx#attachbutton),
which can be used to override the default attach button next to input box as following. We can use this prop to
override default onPress handler for the default attach button.
...
Lets implement the `onPressHandler` to open action sheet showing options to choose attachment from Photo Library, Camera or Files.
We will be using [`@expo/react-native-action-sheet`](https://github.com/expo/react-native-action-sheet)
package for action sheet implementation in this example.
...
Now lets hook the relevant action handlers for buttons in actionsheet.
...
For file attachments, SDK by default uses native file picker. You can reuse that functionality by adding `pickFile` function
provided by [`MessageInputContext`](../contexts/message_input_context.mdx#pickfile) as action handler within actionsheet.
