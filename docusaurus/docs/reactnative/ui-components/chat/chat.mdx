...
`Chat` provides the [ChatContext](../../ui-components/contexts/chat-context), [TranslationContext](../../ui-components/contexts/translation-context.mdx), and [ThemeContext](../../ui-components/contexts/theme-context.mdx) to its child components.
`Chat` is the second highest level component in the Stream Chat for React Native library with only the [`OverlayProvider`](../overlay-provider-components/overlay-provider.mdx) at a higher level.

`Chat` is also responsible for tracking the health of the WebSocket connection with Stream Chat server.
The value `isOnline` provided by the [ChatContext](../../ui-components/contexts/chat-context#isonline) indicates the status of the connection.
...
Detailed information on theming can be found in the [customization documentation](../theme).

:::note

Themes are inherited from parent providers.
A [theme provided to the `OverlayProvider`](../overlay-provider-components/overlay-provider.mdx#value) will be the base theme `style` is merged into.
