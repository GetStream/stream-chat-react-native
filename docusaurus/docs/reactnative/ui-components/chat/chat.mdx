...
`Chat` provides the [ChatContext](../../state-and-offline-support/contexts/chat-context), [TranslationContext](../../state-and-offline-support/contexts/translation-context.mdx), and [ThemeContext](../../state-and-offline-support/contexts/theme-context.mdx) to its child components.
`Chat` is the second highest level component in the Stream Chat for React Native library with only the [`OverlayProvider`](../overlay-provider) at a higher level.

`Chat` is also responsible for tracking the health of the WebSocket connection with Stream Chat server.
The value `isOnline` provided by the [ChatContext](../../state-and-offline-support/contexts/chat-context#isonline) indicates the status of the connection.
