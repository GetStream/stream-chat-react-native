...
Its not possible to replace this component, given the complexity of it. But you can use `imageGalleryCustomComponents` prop on [`OverlayProvider`](./overlay-provider-components/overlay-provider.mdx) to replace sub-parts of image gallery.

Object for providing custom [Image gallery](https://getstream.io/chat/docs/sdk/reactnative/ui-components/image-gallery/) components. Each key in the object is spread as props onto its respective component. Each component has a number of sub-components it can receive in this manner.
...
Partially overrides the `value` provided to the `OverlayContext`. This prop can be used to set the [theme](../theme.mdx) via the `style` key.
