...
Component to render the status of the latest message on a channel within the [`ChannelList`](./channel-list.mdx).

## General Usage

You can customize this component and provide it back to the SDK via the [`PreviewStatus`](./channel-list.mdx#previewstatus) prop on `ChannelList` if desired.
