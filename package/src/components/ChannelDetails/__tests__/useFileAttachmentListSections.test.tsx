...
const messageAt = (id: string, createdAt: string): MessageResponse =>
  ({ created_at: createdAt, id }) as unknown as MessageResponse;
...
describe('useFileAttachmentListSections', () => {
  ...
  it('groups messages into month sections in newest-first order', () => {
    ...
    expect(result.current[0].data.map((m) => m.id)).toEqual(['m-mar']);
    expect(result.current[1].data.map((m) => m.id)).toEqual(['m-feb']);
  });

  it('keeps messages from the same month under a single section', () => {
    ...
    expect(result.current[0].data.map((m) => m.id)).toEqual(['m-2', 'm-1']);
  });
